...
```text
G9-LATAM-TECHMIND-TEAM_79_ATHEN-IA/
├── backend/                  # Código fuente de la API REST y lógica de negocio:es donde recibe ese texto, se lo pasa al modelo de Inteligencia Artificial para que lo analice y le devuelve la respuesta a la pantalla en formato JSON (un formato de texto liviano y organizado).
│   ├── app/                  # Núcleo de la aplicación FastAPI: El "Núcleo de la aplicación FastAPI" es la carpeta donde juntamos la entrada de datos (main.py), la validación (schemas.py) y el procesamiento inteligente 				(services.py) para convertir solicitudes web en respuestas JSON estructuradas.
